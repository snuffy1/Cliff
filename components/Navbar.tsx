...
export default function Navbar() {
  ...
  useEffect(() => {
    if (menuRef.current) {
      gsap.set(menuRef.current, { x: "100%" }); // start off-screen
    }
  }, []);
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-transparent text-white cursor-pointer">
      <div className="max-w-7xl mx-auto py-4 flex items-center justify-between">
        ...
        {/* Right-aligned MENU label and icon in circular background */}
        <div className="flex items-center space-x-3 z-[60]">
          ...
          <button
            ...
            className="bg-[#2a2a2c] rounded-full w-10 h-10 flex items-center justify-center"
          >
            {isOpen ? (
      ...
      <div
        ref={menuRef}
        className="fixed top-0 right-0 w-full h-screen bg-black text-white p-8 pt-24 flex flex-col items-center gap-35 text-2xl transform translate-x-full z-40"
      >
        {links.map((link) => (
          <Link
            key={link.href}
